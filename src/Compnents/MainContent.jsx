    ...
    return (
        ...
        bg-gradient-to-br from-green-50 via-white to-green-100 relative overflow-hidden">
            <video   src="/video.mp4"
                ...
                muted
                playsInline
                className="absolute top-0 left-0 w-full h-full object-cover opacity-65 z-0]"
            >
            </video>
            ...
            <main className="flex flex-col items-center mt-16 animate-fade-in-up w-full px-4 mb-20 z-9">
                <h1 className="text-4xl md:text-5xl font-extrabold text-center text-gray-900 mb-2 tracking-tight animate-slide-in-down">{t('title')}</h1>
                <p className="text-lg md:text-xl text-gray-800 mt-2 text-center animate-fade-in-up">{t('subtitle')}</p>
                ...
                <section className="mt-14 bg-white/60 backdrop-blur-lg shadow-xl rounded-3xl p-10 w-full max-w-md flex flex-col items-center border border-green-100 animate-fade-in-up">
                    <h2 className="text-2xl font-bold mb-4 text-gray-800 animate-slide-in-left">{t('upload')}</h2>
                    ...
                    {preview && (
                        <div className="mt-6">
                            <img src={preview} alt="Uploaded Preview" className="w-40 h-40 object-cover rounded-lg shadow-md" />
                        </div>
                    )}

                    <button onClick={handleDetectDisease} className="mt-6 bg-green-600 text-white rounded-lg px-8 py-2 text-base font-semibold shadow-md transition-all duration-300 hover:bg-green-700 hover:scale-105 focus:outline-none animate-bounce-in">{t('detect')}</button>
                </section>
            </main>
        </div>
    );
